...
async function processCommission(userRef) {
  ...
  if(uh.hasLeft) {
	  ...
	  functions.logger.log("executed left: ",lb);
  }
  if(uh.hasRight) {
	  rb= await processCommission(uh.rightChildRef);
	  functions.logger.log("executed right: ",rb);
  }
  let res;
  if(lb>0 && rb>0) {
	  if(lb>rb) {
  ...
  functions.logger.log("parent executed:: ub: ",ub);
  if(mb>0) {
	res= await processUserCommission(userRef,mb,ub);
    functions.logger.log("parent processed:: mb: ",mb);
  }
  return ub + mb;
}
...
async function processUserCommission(userRef,mb,ub) {
  ...
  functions.logger.log("commission value: ",comVal);
  let res= await setCommissions(userRef,comVal,mb);
  let fub= ub + mb;
  functions.logger.log("calculation value: ",fub);
